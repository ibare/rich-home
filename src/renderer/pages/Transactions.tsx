...
import BudgetSummaryPanel from '../components/shared/BudgetSummaryPanel'
...
export default function Transactions() {
  ...
  const handleBudgetCardDoubleClick = async (budgetItemId: string) => {
    try {
      const result = await budgetRepo.getBudgetItem(budgetItemId) as { id: string; name: string; group_name: string | null; base_amount: number; currency: string; memo: string | null } | undefined
      if (result) {
  ...
  const handleCategoryChange = async (transactionId: string, newCategoryId: string) => {
    try {
      await transactionRepo.updateTransactionCategory(transactionId, newCategoryId)
      loadTransactions()
      loadBudgetSummaries()
    } catch (error) {
  ...
  return (
    <Box>
      ...
      <BudgetSummaryPanel
        ...
        budgetDisplayCurrency={budgetDisplayCurrency}
        onDisplayCurrencyChange={setBudgetDisplayCurrency}
        selectedBudgetItem={selectedBudgetItem}
        onToggleBudgetItem={toggleBudgetItemSelection}
        onBudgetCardDoubleClick={handleBudgetCardDoubleClick}
      />
